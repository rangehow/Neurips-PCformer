...
@register_model("PCformer")
class PCformerModel(FairseqEncoderDecoderModel):
    """
...
class TransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        self.residual_norm = LayerNorm(embed_dim) if self.rk_norm else None
        if self.calculate_num == 2:
            if self.enc_learnable_type == 'gated':
                ...
                if self.alpha_type == 'scalar':
                    if self.layer_wise:
                        self.alpha = torch.nn.Parameter(torch.Tensor(args.encoder_layers, 1))
    ...
    def operator(cur_step, x, residual):
        result = {
            '0' : residual + 1 / 2 * x,
            '1' : residual + 1 / 2 * x,
            '2' : residual + x
    ...
    def forward(
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            for j in range(self.calculate_num):
                ...
                if self.rk_norm:
                    runge_kutta_list.append(self.RK_norm[j](x))
                else:
                ...
                if self.calculate_num == 4:
                    ...
                    x = residual + x
                # add this code for ablation study between 3-order method and 3 NFE predictor-corrector
                elif self.calculate_num == 3:
                    if j == 0:
                        x = residual + 1 / 2 * x
                    elif j == 1:
                        x = residual - runge_kutta_list[0] + 2 * x
            if self.calculate_num == 4:
                if self.enc_learnable_type == 'ema':
                    if self.layer_wise:
                        x = residual + self.alpha[layer_idx] * torch.pow(1-self.alpha[layer_idx],3) * runge_kutta_list[0] + self.alpha[layer_idx] * torch.pow(1-self.alpha[layer_idx],2) * runge_kutta_list[1] + self.alpha[layer_idx] * (1-self.alpha[layer_idx]) * runge_kutta_list[2] + self.alpha[layer_idx] * runge_kutta_list[3]
                    else:
                ...
                if self.enc_learnable_type == 'gated':
                    ...
                    if self.layer_wise:
                        x = residual + self.alpha[layer_idx]*(1-self.alpha[layer_idx]) * runge_kutta_list[0] + self.alpha[layer_idx]*runge_kutta_list[1]
                    else:    
                ...
                x = residual + self.alpha * torch.pow(1-self.alpha,2) * runge_kutta_list[0] + self.alpha * (1-self.alpha) * runge_kutta_list[1] + self.alpha * runge_kutta_list[2]
            else:
...
@register_model_architecture("PCformer", "PCformer")
...
@register_model_architecture("PCformer", "PCformer_iwslt_de_en")
def PCformer_iwslt_de_en(args):
...
@register_model_architecture("PCformer", "PCformer_t2t_iwslt_de_en")
def PCformer_t2t_iwslt_de_en(args):
    ...
    args.encoder_layers = getattr(args, 'encoder_layers', 6)
    args.enc_calculate_num = 2
    PCformer_iwslt_de_en(args)


@register_model_architecture("PCformer", "relative_PCformer_t2t_iwslt_de_en")
def relative_PCformer_t2t_iwslt_de_en(args):
    args.max_relative_length = 8
    args.k_only = True
    args.encoder_layers = 6
    PCformer_t2t_iwslt_de_en(args)

@register_model_architecture("PCformer", "PCformer_wmt_en_de")
def PCformer_wmt_en_de(args):
    args.encoder_history_type = getattr(args, 'encoder_history_type', 'learnable_dense')
    args.decoder_history_type = getattr(args, 'decoder_history_type', 'learnable_dense')
    base_architecture(args)


@register_model_architecture('PCformer', 'PCformer_t2t_wmt_en_de')
def PCformer_t2t_wmt_en_de(args):
    args.encoder_normalize_before = True
    args.decoder_normalize_before = True
    args.attention_dropout = getattr(args, 'attention_dropout', 0.1)
    args.relu_dropout = getattr(args, 'relu_dropout', 0.1)
    args.encoder_history_type = getattr(args, 'encoder_history_type', 'rk_predicotor_multistep_corrector')
    args.decoder_history_type = getattr(args, 'decoder_history_type', 'learnable_dense')
    base_architecture(args)


@register_model_architecture('PCformer', 'relative_PCformer_wmt_en_de')
def relative_PCformer_wmt_en_de(args):
    args.max_relative_length = 8
    args.k_only = True
    PCformer_wmt_en_de(args)


@register_model_architecture('PCformer', 'relative_PCformer_t2t_wmt_en_de')
def relative_PCformer_t2t_wmt_en_de(args):
    args.encoder_layers = getattr(args, "encoder_layers", 6)
    args.enc_calculate_num = getattr(args, 'enc_calculate_num', 4)
    args.max_relative_length = 8
    args.k_only = True
    PCformer_t2t_wmt_en_de(args)


@register_model_architecture('PCformer', 'relative_PCformer_t2t_wmt_en_de_small')
def relative_PCformer_t2t_wmt_en_de_small(args):
    args.encoder_layers = getattr(args, "encoder_layers", 6)
    args.enc_calculate_num = getattr(args, 'enc_calculate_num', 4)
    args.max_relative_length = 8
    args.k_only = True
...
@register_model_architecture('PCformer', 'relative_PCformer_t2t_wmt_en_de_tiny')
def relative_PCformer_t2t_wmt_en_de_tiny(args):
    args.encoder_layers = getattr(args, "encoder_layers", 6)
    args.enc_calculate_num = getattr(args, 'enc_calculate_num', 4)
    args.max_relative_length = 8
    args.k_only = True
...
@register_model_architecture("PCformer", "PCformer_t2t_wmt_en_de_big")
def PCformer_t2t_wmt_en_de_big(args):
...
@register_model_architecture("PCformer", "relative_PCformer_t2t_wmt_en_de_big")
def relative_PCformer_t2t_wmt_en_de_big(args):
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 1024)
    args.encoder_ffn_embed_dim = getattr(args, "encoder_ffn_embed_dim", 4096)
    args.encoder_attention_heads = getattr(args, "encoder_attention_heads", 16)
    args.decoder_embed_dim = getattr(args, "decoder_embed_dim", 1024)
    ...
    args.decoder_attention_heads = getattr(args, "decoder_attention_heads", 16)
    args.dropout = getattr(args, "dropout", 0.3)
    relative_PCformer_t2t_wmt_en_de(args)

@register_model_architecture("PCformer", "relative_PCformer_t2t_wmt_en_de_big_Gemma")
def relative_PCformer_t2t_wmt_en_de_big_Gemma(args):
    ...
    args.encoder_attention_heads = getattr(args, "encoder_attention_heads", 24)
